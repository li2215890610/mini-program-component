Component({
  properties: {
    ...
    placeholder: {
      ...
      value: 0,
      observer: function(newValue) {
        this.refreshPlaceholders(newValue)
      }
    },
    allowHalf: {
      type: Boolean,
  ...
  ready() {
    if (this.data.value === 0 && this.data.placeholder === 0) {
      this.refreshPlaceholders(0)
    }
  },
  methods: {
    refreshValues(value) {
      this.setData({
        values: this.generateArray(Number(value))
      })
    },
    refreshPlaceholders(value) {
      this.setData({
        placeholders: this.generateArray(Number(value))
      })
    },
    generateArray(value) {
      let arr = []
